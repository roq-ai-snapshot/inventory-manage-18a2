...
import { useRoqClient, useSupplyChainFindFirst } from 'lib/roq';
...
function SupplyChainEditPage() {
  ...
  const { data, error, isLoading, mutate } = useSupplyChainFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: SupplyChainInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.supply_chain.update({
        data: values as RoqTypes.supply_chain,
        where: {
          id,
        },
      });
