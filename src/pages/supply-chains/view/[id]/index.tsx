...
import { useSupplyChainFindFirst, useRoqClient } from 'lib/roq';
...
function SupplyChainViewPage() {
  ...
  const queryParams = useMemo<{ include: { inventory: boolean; user: boolean; organization: boolean } }>(
    () =>
      convertQueryToPrismaUtil(
        {
          relations: ['inventory', 'user', 'organization'],
          id,
        },
        'supply_chain',
      ),
  ...
  const { data, error, isLoading } = useSupplyChainFindFirst(queryParams, {}, { disabled: !id });
          ...
          <>
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              ...
              >
                ...
                <FormListItem label="Quantity Supplied" text={data?.quantity_supplied} />

                <FormListItem
                  label="Supply Date"
                  text={data?.supply_date ? format(data?.supply_date, 'dd-MM-yyyy') : ''}
                />
