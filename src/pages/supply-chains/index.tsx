...
import { useRoqClient, useSupplyChainFindManyWithCount } from 'lib/roq';
...
export function SupplyChainListPage(props: SupplyChainListPageProps) {
      ...
      convertQueryToPrismaUtil(
        {
          ...
          searchTermKeys: ['supplier.contains'],
          ...params.filters,
        },
        'supply_chain',
      ),
  ...
  const { data, error, isLoading, mutate } = useSupplyChainFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.supply_chain.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const columns: ColumnType[] = [
    ...
    { id: 'quantity_supplied', header: 'quantity_supplied', accessorKey: 'quantity_supplied' },
    {
      id: 'supply_date',
      header: 'Supply Date',
      accessorKey: 'supply_date',
      cell: ({ row: { original: record } }: any) =>
        record?.supply_date ? format(record?.supply_date, 'dd-MM-yyyy') : '',
    },
